...
public class PhoneARCamera : MonoBehaviour
{
    ...
    private RuntimeReferenceImageLibrary library = imageManager.CreateRuntimeLibrary();
    ...
    private void Awake()
    {
        ...
        {
            GameObject arObject = Instantiate(arGameObject, Vector3.zero, Quaternion.identity);
            arObject.name = arGameObject.name;
            objects.Add(arObject.name, arObject);
        }
    }
    ...
    void OnEnable()
    {
        ...
        if (selected_detector == Detectors.Yolo3_tiny)
        {
    ...
    public void OnRefresh()
    {
        ...
        localization = false;
        staticNum = 0;
        // clear boubding box containers //add check for null
    ...
    {
        trackedImageText.text = trackedImage.referenceImage.name;
    ...
    private void AssignGameObject(string name, Vector3 position)
    {
        if (ARObjects != null)
        {
            ...
            objects[name].transform.position = position;
            objects[name].transform.localScale = scale;
    ...
    private void TFDetect()
    {
        ...
        StartCoroutine(ProcessImage(this.detector.IMAGE_SIZE, result =>
        {
            StartCoroutine(this.detector.Detect(result, boxes =>
            {
                ...
                {
                    StartCoroutine(ProcessImage((int) box.Dimensions.Width, result =>
                    {
                        Texture2D trackingImg = new Texture2D((int)box.Dimensions.Width, (int)box.Dimensions.Width);
                        trackingImg.SetPixels32(result);
                        AddImage(trackingImg, box.Dimensions.Width);
    ...
    private void AddImage(Texture2D image, float width)
    {
