...
public class Yolov4Tiny_Detector : MonoBehaviour, Detector
{
    ...
    private CellDimensions MapBoundingBoxToCell(int x, int y, int box, BoundingBoxDimensions boxDimensions, Dictionary<string, int> parameters)
    {
        ...
        {
            X = (y + Sigmoid(boxDimensions.X)) * parameters["CELL_WIDTH"],
            Y = (x + Sigmoid(boxDimensions.Y)) * parameters["CELL_HEIGHT"],
